...
- **H1 (core edge):** Live ORB entries with setup_score >= 0.45 have positive net expectancy per trade (R > 0, net of execSlippage). **RVOL note (locked 2026-07-12):** The original edge hypothesis referenced RVOL > 2× confirmation. Pre-market RVOL is permanently unavailable at decision time: Yahoo returns zero pre-market volume, and the Robinhood scanner probe (Jul 8–12) confirmed no pre-market data at 5:40am PT. `rvol_spike` is structurally absent for all entries. What N=60 tests is the *RVOL-free variant* of this hypothesis — catalyst-driven gaps that hold the opening range, scored without volume confirmation. This note is pre-registered to close the escape hatch: a negative C3 result cannot be explained away as "we never had RVOL."
...
- **H8 (gap grading composite):** Institutional commitment is measurable post-open via the existing instrumentation: `gapRetained`, catalyst specificity (H4), and sector breadth (H5). These jointly predict ORB pass and subsequent R better than any single filter. This is the framing lens for the N=20 analysis, not a filter to wire. RVOL removed from the composite: pre-market RVOL is permanently unavailable (RH scanner probe closed Jul 12 — no pre-market data). Post-hoc RVOL via `get_equity_historicals` extended bars is a logged-field candidate (backlog #36); if that probe succeeds, RVOL rejoins this composite as an analysis field at C3.
...
Permitted logging-only additions in support: `floatSize` per candidate (FMP), `queueSectorConcentration`, extended screener list (top 25, analysis-only; agent still reads top 10), post-hoc RVOL as a logged field if backlog #36 probe confirms `get_equity_historicals` returns pre-market bars at EOD time.
...
| 2026-07-12 | H1 RVOL note added: pre-market RVOL permanently unavailable (RH scanner probe closed), N=60 tests RVOL-free variant; escape hatch pre-registered closed. H8 updated: RVOL removed from composite pending post-hoc probe (backlog #36). Permitted logging line updated accordingly | (a) | Factual correction — hypothesis must match what the system can actually execute |
| 2026-07-15 | **C1 executed** at N=29 resolved fades. Zero entry-logic changes adopted — no item cleared its pre-stated bar. H3 +2.0 pp (FAILS ≥10 pp); H7 snapshot wins; F2 confirms 10-min window; gapRetained sweep +2.0 pp (FAILS ≥20 pp). H10 first read +0.37R at N=17 (cannot act: N<40, single regime, pre-earnings). Three non-logic action items: (1) fix entryMechanism persistence to trades-log [done this session], (2) log post-6:45am intraday bars for H7 at C2, (3) catalystTag blind audit before C2. **Unused C1-class change does not carry forward** — C2's permitted actions are C2's. Pace clause session count pinned to July 8 2026 ORB regime start (6 sessions as of this date; clause fires at 20 sessions). Earnings season beginning — pace likely to improve. | (a), C1 | C1 checkpoint conclusion |
